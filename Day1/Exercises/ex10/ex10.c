...
main(int argc, char *argv[])
{
  ...
  /*
   * root process: read `nelems' lines of "x.txt" and "y.txt" into arrays `x[]' and `y[]'
   */
  double *x = malloc(nelems*sizeof(double));
  double *y = malloc(nelems*sizeof(double));
  if(rank == 0) {
    readlines(nelems, x, "x.txt");
    readlines(nelems, y, "y.txt");
  }
  ...
  /*
   * MPI_Scatter() to distribute the elements of `x[]' and `y[]' to
   * each process' `x_loc[]' and `y_loc[]' array
   */
  ...
  /*
   * `dot_loc' holds the dot product over each process' local elements
   */
  double dot_loc = 0;
#pragma omp parallel for reduction(+:dot_loc)
  ...
  /*
   * MPI_Reduce() to sum `dot_loc' over all processes and store in
   * `dot' of the root process
   */
  double dot;
  ...
  /*
   * root process: print the timing, the dot product, how many MPI processes and
   * how many OpenMP threads per process were used
   */
#pragma omp parallel
  {
