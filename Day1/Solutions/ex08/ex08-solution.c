...
main(int argc, char *argv[])
{
   ...
   * 0')
   */
  MPI_Sendrecv(&var_proc, 1, MPI_DOUBLE, (rank + 1)%nproc, 0,
	       &var_prev, 1, MPI_DOUBLE, (nproc + rank - 1)%nproc, MPI_ANY_TAG,
   ...
   */
  MPI_Sendrecv(&var_proc, 1, MPI_DOUBLE, (nproc + rank - 1)%nproc, 0,
	       &var_next, 1, MPI_DOUBLE, (rank + 1)%nproc, MPI_ANY_TAG,
	       MPI_COMM_WORLD, MPI_STATUS_IGNORE);
